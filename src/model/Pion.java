...
public class Pion extends AbstractPiece implements Pions {
	...
	public boolean isMoveOk(int xFinal, int yFinal) {
		...
		if ((xFinal == this.getX())
				&& (Math.abs(yFinal - this.getY()) <= 1 || (Math.abs(yFinal
						- this.getY()) <= 2 && this.premierCoup == true))) {

			if ((Couleur.NOIR.equals(this.getCouleur()) && (yFinal
					- this.getY() > 0))
					|| (Couleur.BLANC.equals(this.getCouleur()) && (yFinal
							- this.getY() < 0))) {
				ret = true;
			}
		}
